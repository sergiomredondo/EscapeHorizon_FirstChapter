...
namespace Game.Economy
{
    ...
    public class SH_ResourceSystem : MonoBehaviour
    {
        ...
        public void Initialize(SH_EconomySettings settings)
        {
            ...
            _isInitialized = true;
            _totalDPEarned = PlayerPrefs.GetInt("SH_TotalDPEarned", 0);
        }
        ...
        public void PurgeCores()
        {
            ...
            if (dpGained > 0)
            {
                ...
                PlayerPrefs.SetInt("SH_TotalDPEarned", _totalDPEarned);
                PlayerPrefs.Save();
#if UNITY_EDITOR
                Debug.LogWarning($" [SH_ResourceSystem] Purge successful. DP gained: {dpGained}. Total DP spent: {_totalDPSpent}. Remaining IC: {_currentIdentityCores}. Cost for next DP: {SH_ProgressionCalculator.GetICCostForNextDP(_totalDPSpent, _settings):F1}.");
#endif
            }
            else
            {
        ...
        /// <summary>
        /// Simulates the purge process to calculate how many Development Points would be gained
        /// if the pilot were to purge their current Identity Cores at this moment.
        /// Performs a dry run of the purge logic without modifying any state.
        /// </summary>
        public int CalculatePurgeDPYield()
        {
            if (!_isInitialized || _currentIdentityCores <= 0) return 0;

            int simulatedIC = _currentIdentityCores;
            int simulatedDPSpent = _totalDPSpent;
            int dpYield = 0;

            while (true)
            {
                float cost = SH_ProgressionCalculator.GetICCostForNextDP(simulatedDPSpent, _settings);
                if (simulatedIC < cost) break;
