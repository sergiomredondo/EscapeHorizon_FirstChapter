...
namespace Game.Economy
{
    ...
    public class SH_ResourceSystem : MonoBehaviour
    {
        ...
        public void ApplyDefeatPenalty()
        {
            ...
            int icToRetain = Mathf.FloorToInt(
                _currentIdentityCores * _settings.icDefeatRetentionRate);
            int icLost = icBefore - icToRetain;
            _currentIdentityCores = icToRetain;

            // EC floor: ensure energy does not drop below the guaranteed minimum.
            float energyFloor = _settings.maxEnergy * _settings.energyDefeatFloor;
            float newEnergy = Mathf.Max(_currentEnergy, energyFloor);
            _currentEnergy = newEnergy;

            OnResourceChanged?.Invoke(ResourceType.IdentityCore, _currentIdentityCores);
            OnResourceChanged?.Invoke(ResourceType.EnergyCore, _currentEnergy);
            OnDefeatPenaltyApplied?.Invoke(icLost, _currentIdentityCores, _currentEnergy);
        }
