...
// veerdna.ru modification

void PerlinNoise3D_float(float3 input, float rotateY, out float Out)
{
    // if rotateY  0, dont rotate
    if (rotateY == 0.0)
    {
        Out = cnoise(input);
        return;
    ...
    // confirm rotate
    float3 rotatedInput = mul(rotationMatrix, input);

    // get noise 
    Out = cnoise(rotatedInput);
}
